...
{
    public class SendAutoSmsRefree : IJob
    {
        private StoreDb db = new StoreDb();

        public Task Execute(IJobExecutionContext context)
        {
            var list = db.SuggProfile_SuggType_Interface.Where(s => s.RefreeSms == true).ToList();
            foreach (var item in list)
            {
                string tel = item.SuggestProfil.PersonProfile.Mobile;
                DateTime date;
                string message = "با سلام همکار گرامی" + " \n"+ "نظر داوران برای پیشنهاد شما ثبت گردید. لطفا برای پیگیری پیشنهاد خود، به سامانه نظام پیشنهادها در پورتال شهرداری مراجعه فرمایید" + " "+ "با تشکر از مشارکت شما." + "\n" + "مدیریت نوسازی و تحول اداری-معاونت برنامه‌ریزی و توسعه سرمایه انسانی ";
